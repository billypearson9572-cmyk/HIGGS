...
var CAT_PLURAL = {
  "driveway and paving":  "driveway & paving companies",
  "personal trainers":    "personal training businesses",
  ...
  "kitchen fitters":      "kitchen-fitting businesses",
  "bathroom fitters":     "bathroom-fitting businesses"
};

function catPlural(cat) {
  cat = (cat || "").toLowerCase().trim();
  return CAT_PLURAL[cat] || cat;  // most categories are already plural (builders, roofers, etc.)
}
...
function hook(biz, city, cat, gap, hasWeb) {
  ...
  var plural = catPlural(cat), where = city || "your area";
  if (noSite && rev === 0)
    return "I came across "+biz+" while looking at "+plural+" in "+where+" and noticed you don't have a website yet and very little showing on Google, which usually means ready-to-buy customers are finding competitors first.";
  if (noSite)
    return "I came across "+biz+" while looking at "+plural+" in "+where+". You've clearly got happy customers, but with no website and only a handful of Google reviews you're probably invisible to a lot of people searching right now.";
  if (rev === 0)
    return "I found "+biz+" while looking at "+plural+" in "+where+". You've got a website, but almost no Google reviews showing, often the one thing stopping a business outranking bigger competitors.";
  return "I found "+biz+" while looking at "+plural+" in "+where+". You're set up online, but only a handful of Google reviews showing, usually the main thing holding back where you rank in local search.";
}

function buildBody(biz, city, cat, gap, hasWeb) {
  var plural = catPlural(cat), where = city || "your area";
  return "Hi,\n\n" + hook(biz,city,cat,gap,hasWeb) + "\n\n" +
    "I run " + SENDER_CO + ". We help " + where + " " + plural + " get found online and turn more of those Google searches into booked jobs: a clean website, a stronger Google profile, and a bit of AI automation to chase reviews and answer enquiries automatically.\n\n" +
